...
if training:
    ...
    # agent.load("datanew/weight30.h5")
    # agent.T = 50
    done = False
    batch_size = 50
            ...
            for step in range(0, runsteps):
                ...
                for temp in state.cars:
                    if temp.equals(state.cars[0]):
                        ...
                        temp.updateMotionNew(Message.Message(state.get_Message(temp)), state.cars[0], state.get_numericMessage(temp), False, actiongetted)
                        del curr
                        del actiongetted
