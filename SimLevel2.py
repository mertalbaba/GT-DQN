...
if True:
    ...
    agentlev1.load("datanew/level1/weight39.h5")
    agentlev1.T = 1
    agent = DQNAgent.DQNAgent(state_size, action_size)
    agent.load("datanew/weight49.h5") #142 works well #49 also worked very well
    agent.T = 1
    done = False
    batch_size = 50
    collusion_count = 0
            ...
            for step in range(0, runsteps):
                ...
                for temp in state.cars:
                    if temp.equals(state.cars[0]):
                        ...
                        if actiongetted == 5 or actiongetted == 6:
                            lncount += 1
                        mndist += curr[0, 0, 10]
                        mndistcnt += 1
                        mnspd += temp.velocity_x
                        temp.updateMotionNew(Message.Message(state.get_Message(temp)), state.cars[0],
