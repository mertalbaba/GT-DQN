...
with open('data/realdatai80.txt') as f:
...
for filter_count in range(len(filter_nums)):
    ...
    np.savetxt('results_final_i80/' + str(filter + 1) + '/accmae.txt', np.reshape(np.asarray(acpt), (1, 1)), delimiter='\t')
    np.savetxt('results_final_i80/' + str(filter + 1) + '/rejmae.txt', np.reshape(np.asarray(rjct), (1, 1)), delimiter='\t')
    rescomp2tmp = np.asarray(resultcomp2)
    rescomp3tmp = np.asarray(resultcomp3)
    np.savetxt('results_final_i80/'+str(filter+1)+'/resultcomp2.txt', rescomp2tmp, delimiter='\t')
    np.savetxt('results_final_i80/'+str(filter+1)+'/resultcomp3.txt', rescomp3tmp, delimiter='\t')
